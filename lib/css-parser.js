  ...
  class Node {
    ...
    setRules(newRules, importance) {
      if (this.importance < importance) {
        ...
        this.updateChildrenRules(newRules);
      }
    }

    updateChildrenRules(rules) {
      Object.keys(this.children).forEach(child => {
        this.children[child].setRules(rules, this.importance);
      });
    }
  }
      ...
      while (!this.isOpeningRules()) {
        ...
        if (selector === this.root.type) {
          ...
          if (!root.children[selector]) {
            root.children[selector] = new Node(selector, root.rules);
          }
