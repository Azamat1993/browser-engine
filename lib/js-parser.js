(function(window) {
  ...
  specialForms.fun = function(args, scope) {
    if (args.length === 0) {
      throw new Error("Invalid amount of operators for function");
    }
    const body = args[args.length - 1];

    if (body.type !== "apply") {
      throw new Error("Body of the function should be an apply");
    }
    ...
    args = args.map(arg => {
      if (arg.type !== "word") {
        throw new Error("Every argument of the function should be a word");
      }
    ...
    return function() {
      if (args.length !== arguments.length) {
        throw new Error("Not correct amount of arguments");
      }
